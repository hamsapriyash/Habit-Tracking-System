...
export default function FullStackDashboard() {
  ...
  const [currentDate, setCurrentDate] = useState({ monthDay: "...", weekday: "..." });
  ...
  const submitHabit = async () => {
    ...
    try {
      const userId = localStorage.getItem("userId");
      if (!userId || userId === "undefined") {
        setPopup({ title: "Login Required", message: "Please log in to add a habit.", type: "error" });
        return;
      }
      ...
      if (response.ok) {
        ...
        setPopup({ title: "Failed", message: `Could not save habit: ${errData?.error || response.statusText}`, type: "error" });
      }
    } catch (error) {
      console.error("Fetch Error:", error);
      setPopup({ title: "Error", message: "Error saving to database.", type: "error" });
    }
  };
  ...
  const performDelete = async (id: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setPopup({ title: "Failed", message: "Failed to delete habit.", type: "error" });
      }
    } catch (error) {
      setPopup({ title: "Error", message: "Error deleting habit.", type: "error" });
    }
  };
  ...
  {/* 1. Daily Affirmation (Floating Overlay) */}
              ...
              <div key={habit.id} className="flex flex-col border-b border-slate-100 pb-4 relative group">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => confirmDelete(habit.id)} className="p-2 text-slate-400 hover:text-red-600 bg-slate-50 hover:bg-red-50 rounded-full transition-colors">
      ...
      {showAddCard && (
        <div className="fixed top-0 left-0 right-0 bottom-0 w-full h-full bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9999] px-4">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-sm flex flex-col gap-4">
      ...
      {popup && (
        <div className="fixed top-0 left-0 right-0 bottom-0 w-full h-full bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9999] px-4">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-sm flex flex-col gap-4">
            <h3 className={`text-xl font-bold ${popup.type === 'error' ? 'text-red-600' : 'text-slate-800'}`}>
              {popup.title}
            </h3>
            <p className="text-slate-600">{popup.message}</p>
            <div className="flex justify-end gap-2 mt-2">
              {popup.type === "confirm" && (
                <Button variant="ghost" onClick={() => setPopup(null)}>Cancel</Button>
              )}
              <Button
                className={popup.type === 'error' ? "bg-red-600 hover:bg-red-700 text-white" : "bg-purple-600 hover:bg-purple-700 text-white"}
                onClick={() => {
